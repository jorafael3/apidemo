...
class DemoModel extends Model
{
    ...
    function Principal($CEDULA, $NUMERO, $TERMIMOS)
    {
        ...
        $C = $this->Guardar_Cedula($CEDULA, $NUMERO, $TERMIMOS, $ID_UNICO);
        $EN = $this->OBTENER_ENCRIPT($CEDULA);
        ...
        if ($EN[0] == 1) {
            ...
            $API = $this->CONSULTA_API_REG_DEMOGRAFICO($ENCRY);
            ...
            if ($API[0] == 1) {
                ...
                if (trim($TERMIMOS) == $ESPLENDOR) {
                    $this->Generar_pdf_SPLENDOR($API[1], trim($CEDULA), $NUMERO, $ESPLENDOR);
                }
                if (trim($TERMIMOS) == $HICAR) {
                    $this->Generar_pdf_HICARS($API[1], trim($CEDULA), $NUMERO, $HICAR);
                }
                if (trim($TERMIMOS) == $MISUPERS) {
                    $this->Generar_pdf_MISUPERS($API[1], trim($CEDULA), $NUMERO, $MISUPERS);
                }
                if (trim($TERMIMOS) == $LINEXPRES) {
                    $this->Generar_pdf_MISUPERS($API[1], trim($CEDULA), $NUMERO, $LINEXPRES);
                }
                if (trim($TERMIMOS) == $VITAMINAS) {
                    $this->Generar_pdf_VITAMINAS($API[1], trim($CEDULA), $NUMERO, $LINEXPRES);
                }
                if (trim($TERMIMOS) == $VITAMINAS) {
                    $this->Generar_pdf_BASC($API[1], trim($CEDULA), $NUMERO, $LINEXPRES);
                }
            }


            $this->Actualizar_DatosDemo([$API[1]], $ID_UNICO);
    ...
    function Guardar_Cedula($CEDULA, $NUMERO, $COMERCIO, $ID_UNICO)
    {
        ...
        try {
            ...
            $query = $this->db->connect()->prepare("INSERT INTO encript_agua
        (
            cedula,
            numero,
            comercio,
    ...
    function Actualizar_DatosDemo($DATOS, $ID_UNICO)
    {
        ...
        try {
            ...
            $query = $this->db->connect()->prepare("UPDATE encript_agua
        SET
            datos = :datos
        WHERE 
            ID_UNICO = :ID_UNICO
    ...
    function OBTENER_ENCRIPT($CEDULA)
    {
        ...
        try {
            ...
            while (true) {
                ...
                $query = $this->db->connect()->prepare("SELECT * from encript_agua
    ...
    function Generar_pdf_LINEXPRES($API, $CEDULA, $NUMERO, $EMP)
    {
        $cedula = $API["SOCIODEMOGRAFICO"][0]["IDENTIFICACION"];
        $nombre = $API["SOCIODEMOGRAFICO"][0]["NOMBRE"];
        ...
        de facilitar, promover, permitir o mantener relaciones con el Proveedor de dichos servicios y/o productos,
        quien se considerará como Responsable del tratamiento de mmis datos personales. Consecuentemente,
        reconozco que el Responsable del tratamiento de mis datos es LINEXPRES S.A., con RUC Nro.
        0992599189001, a quien podré contactar al correo: kzambrano@linexpres.com y/o al número telefónico:
        +593 99 883 0735.\n
        3. DATOS PERSONALES: Comprendo y autorizo que, para el cumplimiento del presente instrumento, se
        me podrá solicitar la siguiente información personal: nombres y apellidos, número de identificación,
        dirección de correo electrónico, número telefónico, estado civil, edad, fecha de nacimiento, nacionalidad,
        ...
        proporcionado y que se encuentren en tratamiento. Así mismo, declaro conocer que puedo revocar mi
        consentimiento por escrito, en cualquier momento, sin que sea necesaria una justificación. Para tales fines,
        podré remitir mi requerimiento al correo kzambrano@linexpres.com en concordancia con los
        procedimientos establecidos en la Ley Orgánica de Protección de Datos Personales (LOPDP) y su
        Reglamento, así como las políticas internas de SERVIMATERIALES.
        5. CONSERVACIÓN DE DATOS PERSONALES: SERVIMATERIALES conservará los datos personales durante
        un tiempo no mayor al necesario para cumplir con las finalidades de su tratamiento y el que sea necesario
        ...
        1. DEFINICIÓN Y SERVICIOS: El Chatbot es un asistente virtual operado por SERVIMATERIALES S.A. (En
        adelante SERVIMATERIALES) que permite al USUARIO la consulta de los productos y/o servicios ofertados
        o ya contratados de LINEXPRES S.A., con RUC Nro. 0992599189001, en su calidad de PROVEEDOR. El
        Chatbot no requiere ni procesa el pago de productos y/o servicios; por lo que el USUARIO se abstendrá de
        enviar comprobantes de pago u otras transacciones a través del Chatbot. El uso y/o acceso al Chatbot por
        parte del USUARIO no generará costo alguno de ninguna índole.\n
        2. MANEJO DE LA INFORMACIÓN: Sin perjuicio de los datos personales expresados en la Sección I de este
    ...
    function Generar_pdf_CEL($API, $CEDULA, $NUMERO, $EMP)
    {
        $cedula = $API["SOCIODEMOGRAFICO"][0]["IDENTIFICACION"];
        $nombre = $API["SOCIODEMOGRAFICO"][0]["NOMBRE"];
        ...
        $contenido = utf8_decode("
        Expreso mi consentimiento libre, específico, informado e inequívoco para proporcionar mis datos
        personales y autorizo a la compañía SALVACERO CIA. LTDA., con Registro Único de Contribuyentes (RUC)
        Nro. 0992681845001, (En adelante Encargado del tratamiento) para que efectúe el tratamiento de los
        mismos con las finalidades y en las formas establecidas en el presente instrumento.\n
        1. TRATAMIENTO DE DATOS: Acepto y autorizo para que SALVACERO realice la recogida, recopilación
        procesamiento, obtención, registro, organización, estructuración, conservación, almacenamiento,
        consulta, adaptación, modificación, indexación, extracción, interconexión, utilización, posesión,
        aprovechamiento, acceso, cesión, comunicación, verificación, elaboración de perfiles, transferencia Y
        eliminación de mis datos personales.\n
        Para tales efectos, acepto que SALVACERO podrá gestionar, obtener, cruzar y validar los datos personales
        que he proporcionado con cualquier entidad pública y/o privada que se encuentre debidamente facultada
        en el país, y, en especial, con la Dirección General de Registro Civil, Identificación y Cedulación, la Dirección
        Nacional de Registros Públicos, así como de otras fuentes legales de información autorizadas para operar
        en el país, de acceso público o a las que SALVACERO tenga legítimo acceso, y luego de mi aceptación puedan
        ser registrados para el desarrollo legítimo de la relación jurídica o comercial.
        Comprendo y acepto que mi información personal podrá ser almacenada de manera impresa o digital, y
        accederán a ella los funcionarios de SALVACERO, estando obligados a cumplir con la legislación aplicable a
        las políticas de confidencialidad y protección de datos.\n
        2. FINALIDADES DEL TRATAMIENTO: Conozco y acepto que el tratamiento de mis datos tiene como
        finalidad el procesamiento, análisis, generación de reportes demográficos y de estadísticas, estudios de
        mercado y recepción de anuncios y/o publicidad respecto a los servicios y/o productos que consulte, a fin
        de facilitar, promover, permitir o mantener relaciones con el Proveedor de dichos servicios y/o productos,
        quien se considerará como Responsable del tratamiento de mmis datos personales. Consecuentemente,
        reconozco que el Responsable del tratamiento de mis datos es GIMENEZ VERA JUAN BAUTISTA, con RUC
        Nro. 0906611017001, a quien podré contactar al correo: centlaser@yahoo.com y/o al número telefónico:
        +593 99 975 3120.\n
        3. DATOS PERSONALES: Comprendo y autorizo que, para el cumplimiento del presente instrumento, se
        me podrá solicitar la siguiente información personal: nombres y apellidos, número de identificación,
        dirección de correo electrónico, número telefónico, estado civil, edad, fecha de nacimiento, nacionalidad,
        ...
        proporcionado y que se encuentren en tratamiento. Así mismo, declaro conocer que puedo revocar mi
        consentimiento por escrito, en cualquier momento, sin que sea necesaria una justificación. Para tales fines,
        podré remitir mi requerimiento a centlaser@yahoo.com en concordancia con los procedimientos
        establecidos en la Ley Orgánica de Protección de Datos Personales (LOPDP) y su Reglamento, así como las
        políticas internas de SALVACERO.\n
        5. CONSERVACIÓN DE DATOS PERSONALES: SALVACERO conservará los datos personales durante un
        tiempo no mayor al necesario para cumplir con las finalidades de su tratamiento y el que sea necesario
        para el cumplimiento de disposiciones legales.
        6. MEDIDAS DE SEGURIDAD Y CONFIDENCIALIDAD: Conozco que SALVACERO se compromete a
        implementar y aplicar medidas técnicas y organizativas, adecuadas y necesarias, para la protección de mis
        datos personales, y, especialmente, para evaluar, prevenir, impedir, mitigar y controlar riesgos, amenazas
        y vulnerabilidades; obligación que se hace extensiva al Responsable del tratamiento, de conformidad con
        la LOPDP. Reconozco y acepto que este compromiso no impedirá el tratamiento de mis datos personales
        ...
        $pdf->SetFont('Arial', '', 9);
        $contenido = utf8_decode("
        1. DEFINICIÓN Y SERVICIOS: El Chatbot es un asistente virtual operado por SALVACERO S.A. (En adelante
        SALVACERO) que permite al USUARIO la consulta de los productos y/o servicios ofertados o ya contratados
        de GIMENEZ VERA JUAN BAUTISTA, con RUC Nro. 0906611017001, en su calidad de PROVEEDOR. El
        Chatbot no requiere ni procesa el pago de productos y/o servicios; por lo que el USUARIO se abstendrá de
        enviar comprobantes de pago u otras transacciones a través del Chatbot. El uso y/o acceso al Chatbot por
        parte del USUARIO no generará costo alguno de ninguna índole.\n
        2. MANEJO DE LA INFORMACIÓN: Sin perjuicio de los datos personales expresados en la Sección I de este
        ...
        USUARIO reconoce que, dependiendo del horario en que se realicen las operaciones, estas podrán ser
        procesadas el siguiente día hábil.\n
        4. LÍMITES Y REFORMAS: SALVACERO podrá modificar los límites y condiciones de los servicios brindados
        a través del Chatbot, como: número máximo de consultas, tiempo de conexión, tipo de consultas u
        operaciones que podrán efectuarse, entre otros que SALVACERO determine. Dichos límites y condiciones
        entrarán en vigencia y su aplicación será obligatoria para el USUARIO desde que le fueren comunicados, a
        través del Chatbot u otros canales que, en su momento, estuvieren habilitados. Así mismo, SALVACERO se
        reserva el derecho de modificar los términos y condiciones, los que serán efectivos desde su aceptación
        por parte del USUARIO.\n
        5. MANTENIMIENTO Y SOPORTE: SALVACERO será responsable del funcionamiento del Chatbot, de
        acuerdo a los controles y resguardos físicos y tecnológicos para su uso seguro, considerando los riesgos
        inherentes a su operatividad. Esta responsabilidad no comprende brindar mantenimiento a los dispositivos,
        plataformas, sistemas, aplicaciones o redes desde las cuales opere el Chatbot.
        6. PRIVACIDAD: SALVACERO empleará las medidas necesarias para proteger la integridad de la información
        que le ha sido proporcionada, guardando la confidencialidad de la misma, evitando su divulgación y/o mal
        uso a través de las normas técnicas de seguridad requeridas por la ley y según la naturaleza de la
        información lo amerite. \n
        7. LÍMITE DE RESPONSABILIDAD: SALVACERO no responderá por los daños o perjuicios, directos, indirectos
        o de cualquier naturaleza que el USUARIO pudiera sufrir por las siguientes circunstancias: 7.1. Si la consulta
        u operación requerida por el USUARIO no puede procesarse por no encuadrarse en los parámetros
        prefijados por SALVACERO. En este sentido, el USUARIO reconoce que el Chatbot es un software que ha
        sido programado por SALVACERO bajo cierto formato y parámetros predeterminados; por lo que cualquier
        consulta u operación que no se encuadren en dicho formato y parámetros, no serán procesados. 7.2. Por
        incompatibilidad del dispositivo usado por el USUARIO. 7.3. Por virus, ataques cibernéticos, vulneraciones,
        filtraciones u otras intrusiones de seguridad que afecten la plataforma de mensajería WHATSAPP®. 7.4. Por
        problemas o errores operativos o de conectividad de tal plataforma o por restricciones impuestas por
        autoridades administrativas o judiciales para su funcionamiento. 7.5. Por la sustracción, pérdida, deterioro
        o destrucción del dispositivo desde el cual se accede al Chatbot. 7.6. Por la sustracción de información o
        ingreso de la misma por parte de terceros diferentes al USUARIO, titular de dicha información. SALVACERO
        presume que el USUARIO que realiza la consulta u operación es el titular de la información que proporciona
        y/o requiere o, en su defecto, que ha sido expresamente autorizado por el titular de la misma para
        proporcionarla y/o para recibirla del Chatbot. 7.7. Por la calidad del servicio de telefonía o de Internet
        utilizado para la conectividad y acceso al Chatbot. 7.8. Por el uso o destino que el USUARIO dé a la
        información recibida u operaciones realizadas a través del Chatbot. 7.9. Por la calidad, precios y atención
        respecto a los productos y/o servicios del PROVEEDOR, ya que los mismos no son ofertados por
        SALVACERO. El USUARIO comprende y reconoce que SALVACERO y el PROVEEDOR son personas jurídicas
        distintas y, por ende, no existe relación comercial o vinculación alguna entre ellas para la oferta de los
        productos y servicios que le pertenecen al PROVEEDOR. La intervención de SALVACERO se limita única y
        exclusivamente a la operatividad del Chatbot como medio para de realización de consultas y operaciones
        relacionadas a productos y/o servicios de terceros. 7.10. Por cualquier otro hecho u actuación no imputable
        a SALVACERO. \n
        8. AUTORIZACIÓN: El USUARIO autoriza expresamente a SALVACERO a grabar, captar, registrar y/o
        almacenar los datos y respuestas dadas en el Chatbot, así como las solicitudes, autorizaciones y, en general,
        las instrucciones impartidas a través de tal medio y a reproducirlas y/o mostrarlas en la medida que sea
        necesario aclarar, explicar, demostrar, probar y/o verificar sus actividades en el Chatbot, en especial, ante
        cualquier autoridad fiscal o judicial.\n
    ...
    function Generar_pdf_VITAMINAS($API, $CEDULA, $NUMERO, $EMP)
    {
        $cedula = $API["SOCIODEMOGRAFICO"][0]["IDENTIFICACION"];
        $nombre = $API["SOCIODEMOGRAFICO"][0]["NOMBRE"];
        ...
        de facilitar, promover, permitir o mantener relaciones con el Proveedor de dichos servicios y/o productos,
        quien se considerará como Responsable del tratamiento de mmis datos personales. Consecuentemente,
        reconozco que el Responsable del tratamiento de mis datos es DJANGO S.A., con RUC Nro. 0992732032001,
        a quien podré contactar al correo: rvalencia@django.ec y/o al número telefónico:+593 99 940 0372.\n
        3. DATOS PERSONALES: Comprendo y autorizo que, para el cumplimiento del presente instrumento, se
        me podrá solicitar la siguiente información personal: nombres y apellidos, número de identificación,
        dirección de correo electrónico, número telefónico, estado civil, edad, fecha de nacimiento, nacionalidad,
        idioma de origen, formación o instrucción académica, títulos profesionales, lugar de trabajo, cargo o
        ...
        proporcionado y que se encuentren en tratamiento. Así mismo, declaro conocer que puedo revocar mi
        consentimiento por escrito, en cualquier momento, sin que sea necesaria una justificación. Para tales fines,
        podré remitir mi requerimiento a rvalencia@django.ec en concordancia con los procedimientos
        establecidos en la Ley Orgánica de Protección de Datos Personales (LOPDP) y su Reglamento, así como las
        políticas internas de SALVACERO.\n
        5. CONSERVACIÓN DE DATOS PERSONALES: SALVACERO conservará los datos personales durante un
        tiempo no mayor al necesario para cumplir con las finalidades de su tratamiento y el que sea necesario
        ...
        1. DEFINICIÓN Y SERVICIOS: El Chatbot es un asistente virtual operado por SALVACERO S.A. (En adelante
        SALVACERO) que permite al USUARIO la consulta de los productos y/o servicios ofertados o ya contratados
        de DJANGO S.A., con RUC Nro. 0992732032001, en su calidad de PROVEEDOR. El Chatbot no requiere ni
        procesa el pago de productos y/o servicios; por lo que el USUARIO se abstendrá de enviar comprobantes
        de pago u otras transacciones a través del Chatbot. El uso y/o acceso al Chatbot por parte del USUARIO no
        generará costo alguno de ninguna índole.\n
        2. MANEJO DE LA INFORMACIÓN: Sin perjuicio de los datos personales expresados en la Sección I de este
        instrumento, el Chatbot no solicita información como claves, números de cuentas que el USUARIO
        mantenga en instituciones del Sistema Financiero Nacional, números de tarjetas de crédito o de débito,
        reconocimiento facial, código dactilar, huellas u otros datos de carácter confidencial, personal, financiero
    ...
    function Generar_pdf_BASC($API, $CEDULA, $NUMERO, $EMP)
    {
        $cedula = $API["SOCIODEMOGRAFICO"][0]["IDENTIFICACION"];
        $nombre = $API["SOCIODEMOGRAFICO"][0]["NOMBRE"];
        ...
        de facilitar, promover, permitir o mantener relaciones con el Proveedor de dichos servicios y/o productos,
        quien se considerará como Responsable del tratamiento de mmis datos personales. Consecuentemente,
        reconozco que el Responsable del tratamiento de mis datos es BASC ECUADOR CORPORACION PARA LA
        CALIDAD Y CONTROL CAPITULO GUAYAQUIL, con RUC Nro. 0992164433001, a quien podré contactar al
        correo: coordinador.afiliados@basc-guayaquil.org y/o al número telefónico: +593 98 898 6353.\n
        3. DATOS PERSONALES: Comprendo y autorizo que, para el cumplimiento del presente instrumento, se
        me podrá solicitar la siguiente información personal: nombres y apellidos, número de identificación,
        dirección de correo electrónico, número telefónico, estado civil, edad, fecha de nacimiento, nacionalidad,
        idioma de origen, formación o instrucción académica, títulos profesionales, lugar de trabajo, cargo o
        ...
        proporcionado y que se encuentren en tratamiento. Así mismo, declaro conocer que puedo revocar mi
        consentimiento por escrito, en cualquier momento, sin que sea necesaria una justificación. Para tales fines,
        podré remitir mi requerimiento a coordinador.afiliados@basc-guayaquil.org en concordancia con los
        procedimientos establecidos en la Ley Orgánica de Protección de Datos Personales (LOPDP) y su
        Reglamento, así como las políticas internas de SALVACERO.\n
        5. CONSERVACIÓN DE DATOS PERSONALES: SALVACERO conservará los datos personales durante un
        tiempo no mayor al necesario para cumplir con las finalidades de su tratamiento y el que sea necesario
        para el cumplimiento de disposiciones legales.\n
        6. MEDIDAS DE SEGURIDAD Y CONFIDENCIALIDAD: Conozco que SALVACERO se compromete a
        ...
        1. DEFINICIÓN Y SERVICIOS: El Chatbot es un asistente virtual operado por SALVACERO S.A. (En adelante
        SALVACERO) que permite al USUARIO la consulta de los productos y/o servicios ofertados o ya contratados
        de BASC ECUADOR CORPORACION PARA LA CALIDAD Y CONTROL CAPITULO GUAYAQUIL, con RUC Nro.
        0992164433001, en su calidad de PROVEEDOR. El Chatbot no requiere ni procesa el pago de productos y/o
        servicios; por lo que el USUARIO se abstendrá de enviar comprobantes de pago u otras transacciones a
        través del Chatbot. El uso y/o acceso al Chatbot por parte del USUARIO no generará costo alguno de
        ninguna índole.\n
        2. MANEJO DE LA INFORMACIÓN: Sin perjuicio de los datos personales expresados en la Sección I de este
        instrumento, el Chatbot no solicita información como claves, números de cuentas que el USUARIO
        mantenga en instituciones del Sistema Financiero Nacional, números de tarjetas de crédito o de débito,
